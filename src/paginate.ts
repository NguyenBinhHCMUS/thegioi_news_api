...
export const paginate = async (
  ...
  if (filter && filter.length !== 0)
    filter_fn = generatePaginationFilter(filter);
  // console.log(filter);
  let sort = null;
  ...
  if (projection) {
    ...
    query_fn = model.find(paginatedQuery, select_obj);
  } else query_fn = model.find(paginatedQuery).populate(populate);

  if (sort) {
    query_fn = query_fn.sort({ [sort[0]]: sort[1], _id: -1 });
  }
  query_fn = query_fn.skip(skip).limit(limit);
  ...
  const totalDocs = await model.countDocuments(paginatedQuery);
  return { docs: docs, next_key: next_key, totalDocs };
};
